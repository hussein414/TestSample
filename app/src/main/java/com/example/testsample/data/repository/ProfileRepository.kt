...
class ProfileRepository(private val profileDatabase: ProfileDatabase) {
    ...
    suspend fun deleteProfile(note: ProfileModel) = profileDatabase.getProfileDao().deleteProfile(note)
    suspend fun updateProfile(note: ProfileModel) = profileDatabase.getProfileDao().updateProfile(note)
